      ...
      <div className='bg-white px-4 py-3 max-h-72 overflow-y-auto'>
        ...
        {(testimony.genre || testimony.date) && (
          <div className='flex justify-between items-center pt-2 border-t border-neutral-200 text-[0.75rem]'>
            {testimony.genre && <span className='text-neutral-500'>{testimony.genre}</span>}
            {testimony.date && (
              <span className='font-semibold text-neutral-900'>{testimony.date}</span>
            )}
          </div>
        )}

        {/* Themes */}
        <div className='border-neutral-200 border-t mt-4'>
